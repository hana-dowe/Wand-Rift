...
namespace Unity.FPS.Gameplay
{
    ...
    public class PlayerWeaponsManager : MonoBehaviour
    {
        ...
        public int ActiveSecondaryWeaponIndex {get; private set;}
        ...
        WeaponController[] m_SecondaryWeaponSlots = new WeaponController[3];
        ...
        public bool AddWeapon(WeaponController weaponPrefab)
        {
            ...
            return false;
                ...
                // Adds a weapon to our inventory
        public bool AddSecondaryWeapon(WeaponController weaponPrefab)
        {
           
            // search our weapon slots for the first free one, assign the weapon to it, and return true if we found one. Return false otherwise
            for (int i = 0; i < m_SecondaryWeaponSlots.Length; i++)
            {
                // only add the weapon if the slot is free
                if (m_SecondaryWeaponSlots[i] == null)
                {
                    // spawn the weapon prefab as child of the weapon socket
                    WeaponController weaponInstance = Instantiate(weaponPrefab, WeaponParentSocket);
                    weaponInstance.transform.localPosition = Vector3.zero;
